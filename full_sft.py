"""
MindLM SFT Script
使用混合架构(Attention + Linear Attention) + MoE进行全量微调
"""
...
from MindLM.dataset import PretrainDataset, SFTDataset
...
def train_epoch(epoch, wandb):
    ...
    for step, (X, Y, loss_mask) in enumerate(train_loader):
        ...
        with ctx:
            ...
            # reduction='none' 得到每个token的loss，而不是标量
            pad_id = config.pad_token_id if config.pad_token_id is not None else config.vocab_size - 1
            ...
            # loss_mask过滤掉prompt和padding,只保留answer
        ...
        if (step + 1) % args.save_interval == 0 and (not ddp or dist.get_rank() == 0):
            save_snapshot(epoch, step)


def save_snapshot(epoch, step):
    """定期保存模型快照（覆盖写，固定文件名）"""
    model.eval()
    moe_path = '_moe' if config.use_moe else ''
    linear_path = '_linear' if config.use_linear_attn else ''
    ckpt_path = os.path.join(args.save_dir, f'mindlm_sft_{config.dim}{moe_path}{linear_path}.pth')
...
def save_checkpoint(epoch):
    ...
    moe_path = '_moe' if config.use_moe else ''
    linear_path = '_linear' if config.use_linear_attn else ''
    ckpt_path = os.path.join(args.save_dir, f'mindlm_sft_{config.dim}{moe_path}{linear_path}_epoch{epoch}.pth')
    json_path = os.path.join(args.save_dir, f'mindlm_sft_{config.dim}{moe_path}{linear_path}_epoch{epoch}.json')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_path", type=str,
                        default="data/sft_data_single.csv",
                        help="训练数据路径")
    ...
    if args.resume_from is None:
        Logger("SFT必须通过 --resume_from 指定预训练权重路径")
        sys.exit(1)
